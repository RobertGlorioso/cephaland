    ...
    motion = do
      cmap $ \case
        ...
        (Box (_,w,h), Position p, Velocity v, Gravity g, e, Player) 
          -> (Box (p+v,w,h), 
              if norm v <= playerSpeedLimit + 1
              then (Position $ p + v, Velocity $ v + g)
              else (Position $ p + pure playerSpeedLimit * normalize (v), Velocity $ pure playerSpeedLimit * normalize (v) )
              , e, Player)
        (Box (_,w,h), Position p, Velocity v, Gravity g, e, Enemy) 
          -> (Box (p+v,w,h), 
              if norm v < enemySpeedLimit 
              then (Position $ p + v, Velocity $ v + g)
              else (Position $ p + pure enemySpeedLimit * normalize (v) , Velocity $ pure enemySpeedLimit * normalize (v) )
              , e, Enemy)
